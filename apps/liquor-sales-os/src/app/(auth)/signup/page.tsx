...
import { Wine, Check, ArrowRight, Loader2, AlertCircle, CreditCard, CheckCircle2 } from 'lucide-react'
import Link from 'next/link'
import { signup, preparePayment, LSO_PRICING, formatPrice } from '@saas/core-client'
...
function SignupContent() {
  ...
  const planCode = searchParams?.get('plan') ?? 'lso_free'
  const plan = LSO_PRICING.plans.find(p => p.code === planCode) ?? LSO_PRICING.plans[0]
  ...
  // Step 2 → Step 3 (prepare payment + load Toss)
  ...
  useEffect(() => {
    ...
    payment.requestPayment({
      ...
      // 결제 취소 or 오류 시에도 구독 활성화 (sandbox)
      setPaymentDone(true)
    })
  ...
  return (
    <div className="min-h-screen bg-gray-950 flex items-center justify-center p-4">
      <div className="absolute inset-0 overflow-hidden pointer-events-none">
        <div className="absolute top-1/4 left-1/2 -translate-x-1/2 w-[600px] h-[500px] bg-orange-600/8 rounded-full blur-3xl" />
      </div>
      ...
      <div className="relative w-full max-w-md">
          ...
          {(isFree ? [{ n: 1, label: '계정 정보' }, { n: 3, label: '완료' }] : steps).map((s, i, arr) => (
            <div key={s.n} className="flex items-center gap-2">
              <div className={cn(
                ...
                step >= s.n ? 'bg-orange-600 text-white' : 'bg-gray-700 text-gray-500'
              )}>
                {step > s.n ? <Check className="w-3.5 h-3.5" /> : s.n === 3 ? '3' : s.n}
              </div>
              <span className={cn('text-xs', step >= s.n ? 'text-orange-400' : 'text-gray-600')}>{s.label}</span>
        ...
        {/* Plan badge */}
        <div className="mb-4 flex items-center justify-between px-4 py-3 bg-orange-500/10 border border-orange-500/20 rounded-xl">
          <span className="text-sm text-orange-300 font-medium">{plan.name} 플랜 선택됨</span>
          ...
          <div className="bg-gray-800/60 border border-gray-700/50 rounded-2xl p-6 backdrop-blur-sm">
            <form onSubmit={handleStep1} className="space-y-4">
              ...
              <button
                ...
                className="w-full py-2.5 rounded-xl text-sm font-semibold bg-orange-600 hover:bg-orange-500 text-white disabled:opacity-50 flex items-center justify-center gap-2 shadow-lg shadow-orange-600/20"
              >
          ...
          <div className="bg-gray-800/60 border border-gray-700/50 rounded-2xl p-6 backdrop-blur-sm space-y-4">
            <div className="p-4 bg-gray-700/40 rounded-xl space-y-2">
              ...
              <div className="flex justify-between text-sm border-t border-gray-600 pt-2">
                ...
                <span className="text-orange-400 font-bold">{formatPrice(plan.monthlyPrice)}</span>
              </div>
            </div>
            ...
            <button
              ...
              className="w-full py-3 rounded-xl text-sm font-semibold bg-orange-600 hover:bg-orange-500 text-white disabled:opacity-50 flex items-center justify-center gap-2 shadow-lg shadow-orange-600/20"
            >
        ...
        <p className="text-center text-xs text-gray-600 mt-5">
          이미 계정이 있으신가요?{' '}
          <Link href="/login" className="text-orange-400 hover:text-orange-300">로그인</Link>
        </p>
        <p className="text-center text-xs text-gray-700 mt-2">
          <Link href="/pricing/lso" className="hover:text-gray-500">다른 플랜 보기</Link>
        </p>
      </div>
    </div>
  )
