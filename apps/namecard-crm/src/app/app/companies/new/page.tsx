...
export default function NewCompanyPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    router.push(`/app/companies/${res.data.id}`)
  }
  ...
  return (
    <div className="p-6 max-w-lg">
      <div className="flex items-center gap-3 mb-6">
        ...
        <div>
          <h1 className="text-lg font-bold text-dk-text">회사 추가</h1>
          <p className="text-xs text-dk-muted mt-0.5">회사명만 입력해도 등록됩니다</p>
        </div>
      </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        <div>
          ...
          <input value={form.name} onChange={e => set('name', e.target.value)} placeholder="(주)회사명" className={INPUT_CLS} autoFocus />
        </div>
        <div>
        ...
        <div className="flex gap-3 pt-2">
          <Link href="/app/companies" className="flex-1 py-2.5 text-sm text-center text-dk-muted border border-dk-border rounded-lg hover:bg-dk-surface2 transition-colors">
            취소
          </Link>
          <button
            type="submit"
          ...
          >
            {submitting ? <><Loader2 className="w-4 h-4 animate-spin" />등록 중...</> : '회사 추가'}
          </button>
        </div>
      </form>
    </div>
